...
为防止下载源被篡改，AMCL 对从 GitHub Release 下载的 OpenJDK 资源包执行强制 **SHA-256** 校验。校验值随应用版本固化在源代码中（详见 [`security-signing.md` §6.1](./security-signing.md)）。哈希不匹配时，文件会被自动删除并拒绝加载。
...
| 权限名 | 是否运行时弹窗 | 用途 | 不授予的影响 |
|---|---|---|---|
| `ohos.permission.INTERNET` | 否（普通权限） | 与 §2.3 所列第三方官方服务器通信，下载 OpenJDK / MC 资源、完成正版登录 | 完全无法使用本应用 |

我们承诺**不会**在不告知您的前提下追加权限。任何后续版本若新增权限，会在本政策更新记录中明确披露。

> **关于 ELF Loader 与 ACL 受限权限**：
> AMCL 包含一个自实现的 ELF 动态库加载器，用于加载从 GitHub Release 下载到沙箱的 OpenJDK 运行时（绕过 HarmonyOS 内核 `MAP_XPM` 对沙箱内可执行内存的代码签名约束）。该加载器**仅**加载经 SHA-256 校验的、由项目维护者在公开 Docker 容器中编译产出的 OpenJDK 二进制，**不**提供任意 URL 下载并执行代码的能力，**不**加载用户自带的 `.so`，**不**访问任何额外系统能力。该机制的合规边界与工程约束详见 [`security-signing.md` §7](./security-signing.md)。
...
- **个人信息保护负责人邮箱**：*待开发者补充*（建议在提交华为应用市场前填写）
- **通讯地址**：*待开发者补充*
